...
def assignCluster(dataSet, k, centroids):
    ...
    for dataPoint in dataSet:
        ...
        for i in range(k):                                                              
            dist = np.linalg.norm(centroids[i] - dataPoint)
            distList.append(dist)
        # assign dataPoint to closest cluster    
        clusterAssment[index] = (np.where(distList == np.amin(distList))[0])[0] + 1     
        index += 1
...
def getCentroid(dataSet, k, clusterAssment):
    ...
    '''
    centroids = []
    for clusterID in range(1,k+1):
        clusterIndices = []                                 # indices of dataPts that belong to a cluster
        newCentroid = []                                    # new centroid
        for dataPtIndex in range(len(clusterAssment)):
            if clusterID == clusterAssment[dataPtIndex]:
                clusterIndices.append(dataPtIndex)
        cluster = dataSet[clusterIndices, :]
        for i in range(cluster.shape[1]):
            newCentroid.append(mean(cluster.T[i]))
